...
static struct device* fw_sysfs_device = NULL;
...
static int __init my_module_init_function(void) {
	...
	fw_sysfs_device = device_create(fw_sysfs_class, NULL, MKDEV(major_number, 0), NULL, "fw_sysfs_device");
	if(IS_ERR(fw_sysfs_device)) {
		kfree(nf_net_forward_hook);
		kfree(nf_net_local_in_hook);
		kfree(nf_net_local_out_hook);
		nf_unregister_net_hook(&init_net, nf_net_forward_hook);
	...
	if(device_create_file(fw_sysfs_device, (const struct device_attribute *)&dev_attr_fw_sysfs_att_dropped_packets_counter.attr)){
		kfree(nf_net_forward_hook);
		kfree(nf_net_local_in_hook);
		kfree(nf_net_local_out_hook);
		nf_unregister_net_hook(&init_net, nf_net_forward_hook);
	...
	if(device_create_file(fw_sysfs_device, (const struct device_attribute *)&dev_attr_fw_sysfs_att_accpeted_packets_counter.attr)){
		kfree(nf_net_forward_hook);
		kfree(nf_net_local_in_hook);
		kfree(nf_net_local_out_hook);
		nf_unregister_net_hook(&init_net, nf_net_forward_hook);
		...
		device_remove_file(fw_sysfs_device, (const struct device_attribute *)&dev_attr_fw_sysfs_att_dropped_packets_counter.attr);
		device_destroy(fw_sysfs_class, MKDEV(major_number, 0));
		class_destroy(fw_sysfs_class);
		unregister_chrdev(major_number, "fw_sysfs_device");
		return -1;
...
static void __exit my_module_exit_function(void) {
	...
	device_remove_file(fw_sysfs_device, (const struct device_attribute *)&dev_attr_fw_sysfs_att_dropped_packets_counter.attr);
	device_remove_file(fw_sysfs_device, (const struct device_attribute *)&dev_attr_fw_sysfs_att_accpeted_packets_counter.attr);
	device_destroy(fw_sysfs_class, MKDEV(major_number, 0));
	class_destroy(fw_sysfs_class);
	unregister_chrdev(major_number, "fw_sysfs_device");
}
